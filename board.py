...
class Board(GridLayout):
    ...
    def __init__(self, card_click_handler, cb_after_faceup_delay, **kwargs):
        ...
        self.__board = GridLayout(cols=5, size_hint_y=1, spacing=5, padding=5)
        self.__board.bind(size=self.board_on_size)
        self.__filler = Filler(cols=1, size_hint_y=1)
        self.add_widget(self.__board)
    ...
    def board_on_size(self, *args):
        print("Board.board_on_size", len(self.__board.children))
        with self.__board.canvas.before:
            Color(rgb=get_color_from_hex("#9ACD32")) # "#777700"))
            Rectangle(pos=self.__board.pos, size=self.__board.size)
    ...
    def set_board(self, board):
        ...
        for i, card in enumerate(board):
            ...
            self.__board.add_widget(Card(card_name, self.card_click_handler, i), index=i)
        self.__filler = Filler(cols=1, size_hint_y=filler_size_hint_y)
        self.add_widget(self.__filler)

    def render_game_state(self, game_state):
        """render_game_state
        game_state = {}, the fields of the state are set in game_logic-module
        """
        removed = 1
        pair_found = False
        ...
        for gl_card in game_state["faceUp"]:
            ...
            ui_card = self.__board.children[card_ind]
            ui_card.turn_up()
            if removed == 1:
