...
class Controller(Widget):
    ...
    def _recv(self, *args):
        ...
        # print("_recv")
        while ws.recv():
            if "message" in ws.store:
                #print("_recv:", store["message"])
                try:
                    ws_message = json.loads(ws.store["message"])
                except json.JSONDecodeError:
                    print("Controller._recv", "JSONDecodeError", ws.store["message"])
                else:
                    message_type = ws_message["type"]
                    if message_type is None:
                        print("Controller._recv", "no type in message", ws.store["message"])
                    else:
                        if message_type in (MESSAGE_TYPE_START_GAME, MESSAGE_TYPE_GAME_STATE,\
                            ...
                            try:
                                ...
                                return
                        print("Contoller._recv", ws.store["message"])
